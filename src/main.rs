...
async fn run_command(
    ...
    let unsupported_dry_run_command = match &command {
        ...
        Commands::Milestones {
            action: commands::milestones::MilestoneCommands::Delete { .. },
        } => Some("milestones"),
        Commands::Issues { action }
            if !matches!(
                action,
                issues::IssueCommands::Create { .. }
                    | issues::IssueCommands::Update { .. }
                    | issues::IssueCommands::List { .. }
                    | issues::IssueCommands::Get { .. }
                    | issues::IssueCommands::Open { .. }
                    | issues::IssueCommands::Link { .. }
            ) =>
        {
            Some("issues")
        }
        Commands::Relations { action }
            if !matches!(action, relations::RelationCommands::List { .. }) =>
        {
            Some("relations")
        }
        Commands::Sprint {
        ...
        Commands::Api {
            action: commands::api::ApiCommands::Mutate { .. },
        } => Some("api mutate"),
        ...
        Commands::Cache {
            action: commands::cache::CacheCommands::Clear { .. },
        } => Some("cache"),
        Commands::Config {
            action:
    ...
    if let Some(command_name) = unsupported_dry_run_command {
